#!/usr/bin/env python3
"""
批量生成不同嵌入率的 LSBM stego 图像。

输出目录：./LSBM
输出文件名格式：lsbm_stego_{ratio:.1f}.png （例如 lsbm_stego_0.1.png）

使用：在 exp2 目录下运行 `python generate_lsbm_batch.py`
"""
import os
...
def main():
    ...
    for r in ratios:
        ...
        # 调用 lsbm 的嵌入函数
        stego = lsbm.lsbm_embed(embed_bits, cover.copy(), seed=seed)

        out_name = f'lsbm_stego_{r:.1f}.png'
