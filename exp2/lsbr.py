...
def lsbr_embed(
    ...
    indices = rng.choice(pixel_cnt, size=len(msg_bits), replace=False) # 随机选择像素点嵌入
...
def lsbr_extract(
    ...
    rng = np.random.default_rng(seed)
    indices = rng.choice(pixel_cnt, size=bit_length, replace=False) # 根据seed复现随机位置
    msg_bits = []
    for idx in indices:
...
def main(args):
    ...
    stego = lsbr_embed(embed_bits, cover, seed=args.seed)
    ...
    extract_bits = lsbr_extract(bit_length, stego, seed=args.seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stego_path', type=str, default='res/lena_gray_lsbr_stego.png')
    parser.add_argument('--seed', type=int, default=2025)
    parser.add_argument('--ratio', type=float, default=1.0)
    parser.add_argument('--vis_pos_path', type=str, default='res/lena_gray_lsbr_msg_pos.png')
    parser.add_argument('--vis_hist_path', type=str, default='res/lena_gray_lsbr_msg_hist.png')
    parser.add_argument('--vis_diff_path', type=str, default='res/lena_gray_lsbr_msg_hist_diff.png')
